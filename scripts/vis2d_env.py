"""生成可视化对比图: SAC vs Closest 前沿排序 (2D 俯视图, 使用 SequenceEnv)."""
...
def run_with_env(env, max_steps, model=None, seed=0):
    ...
    for step in range(max_steps):
        ...
        progress = env.core.get_exploration_progress()
        frontiers = env._cached_frontiers
    ...
    final_cov = env.core.get_exploration_progress()
...
def plot_comparison(seed, max_steps, model_path, out_dir):
    ...
    for label, use_model in [("Closest", False), ("SAC", True)]:
        env = SequenceEnv(max_steps=max_steps, num_pillars=15)
        m = model if use_model else None
        path, snaps, cov, dist, env_final = run_with_env(env, max_steps, model=m, seed=seed)
    ...
    for ax, (label, res) in zip(axes, results.items()):
        env_vis = res["env"]
        slice_2d = np.array(env_vis.core.get_occupancy_slice_2d(1.5)).reshape(200, 200)
    ...
    plt.suptitle(f"Exploration Curves (seed={seed})", fontsize=13, fontweight="bold")
    plt.tight_layout()
