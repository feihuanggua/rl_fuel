"""SAC Seq10: Continue from seq8 best, pure ARiADNE, larger buffer.

- Load seq8 checkpoint as starting point
- Only ARiADNE maps (no random pillars)
- Larger buffer (200k) to retain hard map experience
- 500k steps total
- Smaller lr (5e-5) for fine-tuning
"""
...
def train_sac_seq10(args):
    ...
    print(f"[SEQ10] start={start_step}, total={args.total_steps}, pure ARiADNE, lr={args.lr}, buffer={args.buffer_size}", flush=True)
        ...
        if step % 5000 == 0 and step > 0:
            ...
            for mp_name in ARIADNE_MAPS:
                ...
                for es in range(500):
                    ...
                    if nv == 0:
                        eval_results.append((mp_name, eenv._get_exploration_progress()))
                        break
                    ...
                    if ed:
                        eval_results.append((mp_name, einfo.get("coverage", 0)))
                        break
                else:
                    eval_results.append((mp_name, eenv._get_exploration_progress()))

            actor.train()
            avg_eval = np.mean([c for _, c in eval_results])
            per_map = " ".join([f"{n}={c:.1%}" for n, c in eval_results])
            print(f"  [EVAL step={step}] avg={avg_eval:.3f} | {per_map}")
            tb.add_scalar("eval_coverage_avg", avg_eval, step)

            if avg_eval > best_eval:
                best_eval = avg_eval
                torch.save(actor.state_dict(), os.path.join(args.save_dir, "best_actor.pth"))
                print(f"  [NEW BEST eval={best_eval:.3f}]")
    ...
    print(f"Done. Best eval avg: {best_eval:.3f}")


def main():
    parser = argparse.ArgumentParser(description="SAC Seq10: Fine-tune on pure ARiADNE")
    parser.add_argument("--save-dir", type=str, default="./fuel_rl_checkpoints/sac_seq10")
