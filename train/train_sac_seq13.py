"""SAC Seq13: Large ARiADNE maps (40m max), 4m ray, fresh training.

- Pure ARiADNE maps (exclude 2.png, 3.png)
- 4m ray distance, 0.1m resolution
- max_steps=800, lr=1e-4, buffer=200k
- 500k steps
- Per-map eval with coverage + distance
"""
...
def train_sac(args):
    ...
    print(f"[SEQ13] start={start_step}, total={args.total_steps}, 4m ray, 40m maps, lr={args.lr}", flush=True)
    ...
    for step in range(start_step, args.total_steps + 1):
        ...
        if step % 5000 == 0 and step > 0:
            ...
            for mp_name in EVAL_MAPS:
                ...
                eenv = SequenceEnv(max_steps=500, map_size=(50, 40, 2),
                                   map_type='ariadne', map_path=mp)
...
def main():
    parser = argparse.ArgumentParser(description="SAC Seq13: Large ARiADNE 4m ray")
    parser.add_argument("--save-dir", type=str, default="./fuel_rl_checkpoints/sac_seq13")
