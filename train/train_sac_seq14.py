"""SAC Seq14: Self-attention model + auto-alpha + improved training.

Changes from seq13:
- Model: FrontierEncoder with self-attention, d_hidden=256
- Obs: relative position, fixed normalization for 40m maps
- Training: auto alpha, critic update every step, actor every 2
- Eval: max_steps=800 (same as training)
- Reward clip: [-20, 20]
"""
import os, sys, csv, argparse, time, random
import numpy as np
import torch
...
ARIADNE_MAPS = ["1.png", "4.png", "5.png", "10.png", "20.png", "50.png", "100.png"]
EVAL_MAPS = ["1.png", "2.png", "3.png", "4.png", "5.png",
             "10.png", "20.png", "50.png", "100.png"]


def make_env(max_env_steps=800):
    ...
    return SequenceEnv(max_steps=max_env_steps, map_size=(50, 40, 2),
...
class QNetwork(nn.Module):
    ...
    def forward(self, frontiers, mask, global_feat, map_img=None):
        feat, _ = self.encoder(frontiers, mask, global_feat, map_img)
        q = self.q_net(feat).squeeze(-1)
...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        return (frontiers, mask, global_f, map_imgs, actions, rewards,
                n_frontiers, n_mask, n_global, n_map, dones)
...
def train_sac(args):
    ...
    env = make_env(args.max_env_steps)
    ...
    target_entropy = np.log(MAX_FRONTIERS) * args.entropy_ratio
    ...
    print(f"[SEQ14] start=1, total={args.total_steps}, lr={args.lr}, "
          f"target_entropy={target_entropy:.2f}", flush=True)
    ...
    for step in range(1, args.total_steps + 1):
        ...
        if len(buffer) >= args.batch_size:
            (s_f, s_m, s_g, s_map, a, r,
             ns_f, ns_m, ns_g, ns_map, d) = buffer.sample(args.batch_size)
            ...
            with torch.no_grad():
                ...
                v_next = (ns_probs * (t_min - alpha * ns_logprobs)).sum(
                    dim=-1, keepdim=True)
            ...
            nn.utils.clip_grad_norm_(
                list(critic1.parameters()) + list(critic2.parameters()), 1.0)
            critic_opt.step()

            update_count += 1

            # --- Actor (every 2 critic updates) ---
            if update_count % 2 == 0:
                ...
                actor_loss = (a_probs * (alpha * a_logprobs - c_min)).sum(
                    dim=-1).mean()
                actor_opt.zero_grad()
                actor_loss.backward()
                ...
                # --- Alpha ---
                with torch.no_grad():
                    entropy = -(a_probs * a_logprobs).sum(dim=-1).mean()
        ...
        if step % args.log_every == 0:
            ...
            tb.add_scalar("train/alpha", alpha, step)

        # === Eval ===
        if step % 5000 == 0 and step > 0:
            actor.eval()
            maps_dir = os.path.join(
                os.path.dirname(__file__), "..", "..", "maps", "ariadne")
            eval_results = []
            for mp_name in EVAL_MAPS:
                mp = os.path.join(maps_dir, mp_name)
                eenv = SequenceEnv(max_steps=args.max_env_steps,
                                   map_size=(50, 40, 2),
                                   map_type='ariadne', map_path=mp)
                eobs, _ = eenv.reset(seed=42)
                for es in range(args.max_env_steps):
                    nv = int(eobs["mask"].sum())
                    if nv == 0:
                        eval_results.append(
                            (mp_name, eenv._get_exploration_progress(),
                             eenv.total_distance))
                        break
            ...
            valid_covs = [c for n, c, d in eval_results
                          if n not in ("2.png", "3.png")]
            avg_valid = np.mean(valid_covs)
            per_map = " ".join(
                [f"{n}={c:.0%}/{d:.0f}m" for n, c, d in eval_results])
            print(f"  [EVAL step={step}] avg_valid={avg_valid:.3f} | {per_map}")
            tb.add_scalar("eval_avg_valid", avg_valid, step)

            if avg_valid > best_eval:
                best_eval = avg_valid
                torch.save(actor.state_dict(),
                           os.path.join(args.save_dir, "best_actor.pth"))
                print(f"  [NEW BEST valid={best_eval:.3f}]")

        # === Checkpoint ===
        if step % 1000 == 0:
            torch.save(actor.state_dict(),
                       os.path.join(args.save_dir, f"actor_{step}.pth"))
            torch.save({
                "step": step,
    ...
    csv_f.close()
    torch.save(actor.state_dict(),
               os.path.join(args.save_dir, "final_actor.pth"))
    print(f"Done. Best valid avg: {best_eval:.3f}")


def main():
    parser = argparse.ArgumentParser(description="SAC Seq14: Attention + AutoAlpha")
    parser.add_argument("--save-dir", type=str,
                        default="./fuel_rl_checkpoints/sac_seq14")
    parser.add_argument("--total-steps", type=int, default=500000)
    parser.add_argument("--buffer-size", type=int, default=200000)
    parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--gamma", type=float, default=0.99)
    parser.add_argument("--entropy-ratio", type=float, default=0.5,
                        help="target entropy = ratio * log(MAX_FRONTIERS)")
