"""SAC Curriculum Learning: easy -> medium -> hard ARiADNE maps.

Phase 1 (0-50k):    maps 1,2 + random pillars
Phase 2 (50k-150k): maps 1,2,3,4,5 + random pillars
Phase 3 (150k-300k): all 9 maps + random pillars
"""
...
from fuel_rl.map_loader import list_ariadne_maps


CURRICULUM = {
    1: ["1.png", "2.png"],
    2: ["1.png", "2.png", "3.png", "4.png", "5.png"],
    3: ["1.png", "2.png", "3.png", "4.png", "5.png",
...
def make_env_curriculum(step, max_env_steps=800, num_pillars=15):
    ...
    if np.random.random() < 0.6:
        ...
        return SequenceEnv(max_steps=max_env_steps, num_pillars=num_pillars,
                           map_size=(20, 20, 2))
...
def train_sac_curriculum(args):
    os.makedirs(args.save_dir, exist_ok=True)
    env = make_env_curriculum(1, args.max_env_steps, args.num_pillars)
    ...
    if start_step == 1:
        csv_w.writerow(["step", "avg_r", "coverage", "alpha", "phase"])
    ...
    prev_phase = get_curriculum_phase(start_step)
    print(f"[CURRICULUM] start_step={start_step}, total={args.total_steps}, initial_phase={prev_phase}", flush=True)

    for step in range(start_step, args.total_steps + 1):
        phase = get_curriculum_phase(step)
        if phase != prev_phase:
            print(f"\n[CURRICULUM] Phase {prev_phase} -> {phase} at step {step}", flush=True)
            prev_phase = phase
        ...
        with torch.no_grad():
            ...
            if n_valid == 0:
                n_skip += 1
                env = make_env_curriculum(step, args.max_env_steps, args.num_pillars)
                obs, _ = env.reset()
                n_ep += 1
                ep_rew = 0.0
                continue
        ...
        if step % args.log_every == 0:
            ...
            print(f"Step {step:6d}: cov={info.get('coverage',0):.3f} "
                  ...
                  f"phase={get_curriculum_phase(step)} skip={n_skip} eps={n_ep} "
                  f"mem={mem_gb:.1f}GB "
                  f"time[act={t0_act:.1f}s env={t0_env:.1f}s upd={t0_upd:.1f}s]")
            sys.stdout.flush()
            t0_act = t0_env = t0_upd = 0.0
            csv_w.writerow([step, reward, info.get("coverage", 0), alpha, get_curriculum_phase(step)])
            csv_f.flush()
            tb.add_scalar("train/coverage", info.get("coverage", 0), step)
            tb.add_scalar("train/reward", reward, step)
            tb.add_scalar("train/phase", get_curriculum_phase(step), step)
        ...
        if step % 5000 == 0 and step > 0:
            ...
            for _ in range(5):
                ...
                for es in range(400):
                    ...
                    if nv == 0:
                        eval_covs.append(env._get_exploration_progress())
                        break
            ...
            print(f"  [EVAL step={step} phase={get_curriculum_phase(step)}] det_cov={avg_eval:.3f} over {len(eval_covs)} eps")
...
def main():
    parser = argparse.ArgumentParser(description="SAC Curriculum Learning")
    parser.add_argument("--save-dir", type=str, default="./fuel_rl_checkpoints/sac_seq9")
