#ifndef SDF_MAP_STANDALONE_H
#define SDF_MAP_STANDALONE_H
...
struct MapData {
  ...
  std::vector<short> count_hit_, count_miss_, count_hit_and_miss_;
  std::vector<char> flag_rayend_, flag_visited_;
  char raycast_num_;
...
public:
  ...
  void init(const SDFMapParams& params);
  void inputPointCloud(const std::vector<Eigen::Vector3d>& points, const Eigen::Vector3d& camera_pos);
  void injectObstacles(const std::vector<Eigen::Vector3d>& points);

  // Fill all voxels at z <= ground_z as occupied within the exploration box
  void addGroundPlane(double ground_z);
  ...
  // RL-specific: get local voxel grid for observation
  std::vector<int8_t> getLocalVoxelGrid(const Eigen::Vector3d& center, int grid_size);

  // RL-specific: get 2D occupancy slice at given height
  std::vector<int8_t> getOccupancySlice2D(double height);

  // RL-specific: count voxels by type
  int countUnknownVoxels() const;
  int countFreeVoxels() const;
  int countOccupiedVoxels() const;
  double getExplorationProgress() const;

  // Accessors for dependent modules
  std::unique_ptr<MapParam>& mp() { return mp_; }
  std::unique_ptr<MapData>& md() { return md_; }
  RayCaster& caster() { return *caster_; }
